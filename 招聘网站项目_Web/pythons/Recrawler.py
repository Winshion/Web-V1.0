...
def list_fulfilling(job_info_list, job_name=[], company_name=[], company_type=[],
    ...
    添加元素，不删除原表单数据。
    """
    education_lst = ["初中", "中专", "中技", "高中", "本科", "大专", "硕士", "博士"]
    for each in job_info_list:
        ...
        if (each["attribute_text"][-2]) in education_lst:
            education.append(each["attribute_text"][-2])
        else:
            education.append('无')
    return job_name, work_city, company_name, company_type, company_size, company_ind, date, education
